var xq = function () {
    ...
    function drawCellBoard() {
        ...
        for (y = 0; y < 10; y++) {
            for (x = 0; x < 9; x++) {
                idx = xyToIndex(x, y);
                c = curXQPos.cell(idx);
                if (c && !((curState === STAT.AIING || curState === STAT.AI_RETURNED) && curMove && from === idx)) {
                    ti = CELL2TILEINDEX[c];
    ...
    function loadAllResource() {
        ...
        tileImage = document.getElementById("img_xq")
        boardImage = document.getElementById("img_board")
        showLoading(100)
    ...
    function tickTimer() {
        if (curState !== STAT.AIING && curState !== STAT.AI_RETURNED)
            return;
